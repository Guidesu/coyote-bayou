...
/obj/item/validball/proc/activate_the_validball()
	...
	to_chat(world, span_danger("Be it by overheard rumor, or a sudden memory, you realize that the coveted keycards recently sent out their detection information. The hunt is on, for those that want to be a part of it. You can pick up a device to help find it in one of Mankara's shops vending machines. But remember, those who hunt open themselves up to a world of danger."))
...
/obj/item/pinpointer/validball_finder/proc/z2text(turf/hereturf)
	...
	switch(hereturf.z)
		...
		if(Z_LEVEL_NASH_UNDERGROUND)
			return "Mankara Wastes - Underground"
		if(Z_LEVEL_NASH_COMMON)
			return "Mankara Wastes - Common"
		if(Z_LEVEL_NASH_LVL2)
			return "Mankara Wastes - Second Story"
		if(Z_LEVEL_NASH_LVL3)
			return "Mankara Wastes - Third Story"
		if(Z_LEVEL_REDWATER)
